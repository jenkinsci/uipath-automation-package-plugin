...
public final class CliConfiguration {
    ...
    private static CliConfiguration INSTANCE = null;
    public static final String GLOBAL_SELECTED_CLI_VERSION_KEY = "SELECTED_CLI_VERSION_KEY";
    ...
    * */
    private CliConfiguration() throws JsonProcessingException {
    ...
    public synchronized static CliConfiguration getInstance() throws JsonProcessingException {
        if (INSTANCE == null) {
    ...
    public Map<String,Configuration> getConfiguration() {
        ...
        this.cliConfigurationMap.forEach((versionKey,cliConfig)->{
    ...
    public String getDefaultCliVersionKey() {
        return CliConfiguration.WIN_PLATFORM + "_" + ResourceBundle.getBundle("config").getString(CliConfiguration.DEFAULT_CLI_VERSION_KEY);
    }
    ...
    public Optional<FilePath> getCliRootCachedDirectoryPath(@Nonnull Launcher launcher, @Nonnull EnvVars env, String cliVersionKey) throws IOException, InterruptedException {
        ...
        return Optional.of(cachedRootPath);
    }

    public Optional<FilePath> getCliRootDownloadsDirectoryPath(@Nonnull Launcher launcher, @Nonnull EnvVars env, String cliVersionKey) throws IOException, InterruptedException {
        if(!cliConfigurationMap.containsKey(cliVersionKey)) {
            return Optional.empty();
        }
        ...
        return Optional.of(downloadsRootPath);
    }
    ...
    public Optional<FilePath> getCliPath(@Nonnull Launcher launcher, @Nonnull EnvVars env, String cliVersionKey) {
        ...
        try{
            Optional<FilePath> cachedRootPath = getCliRootCachedDirectoryPath(launcher, env, cliVersionKey);
            if(cachedRootPath.isPresent()) {
                Configuration configuration = cliConfigurationMap.get(cliVersionKey);
                FilePath cliCachedPath = cachedRootPath.get();
